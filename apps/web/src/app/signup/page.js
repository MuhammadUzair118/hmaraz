...
function getPasswordStrength(password) {
    ...
    if (password.length < 6)
        return { label: 'Weak', color: 'bg-danger', width: '25%' };
    ...
    return (<div className="flex min-h-screen flex-col items-center justify-center bg-bg px-4">
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4">
        <div>
          ...
          <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} className={`w-full rounded-lg border px-4 py-2.5 text-sm focus:outline-none ${errors.name ? 'border-danger focus:border-danger' : 'border-gray-300 focus:border-primary'}`} placeholder="Ahmed Raza"/>
        ...
        <div>
          ...
          <div className="relative">
            <input id="confirmPassword" type={showConfirm ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={`w-full rounded-lg border px-4 py-2.5 pr-10 text-sm focus:outline-none ${errors.confirmPassword ? 'border-danger focus:border-danger' : 'border-gray-300 focus:border-primary'}`} placeholder="••••••••"/>
            <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-gray hover:text-dark-slate">
              {showConfirm ? <lucide_react_1.EyeOff size={18}/> : <lucide_react_1.Eye size={18}/>}
            </button>
          </div>
        ...
        <div className="flex items-start gap-2">
          <input id="terms" type="checkbox" checked={agreeTerms} onChange={(e) => setAgreeTerms(e.target.checked)} className="mt-1 h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"/>
        ...
        <p className="mt-6 text-center text-sm text-muted-gray">
          ...
          <link_1.default href="/signin" className="font-medium text-primary hover:underline">
            Sign in
          </link_1.default>
        </p>
      </form>
